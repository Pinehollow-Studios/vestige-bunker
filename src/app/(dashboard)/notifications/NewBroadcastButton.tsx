...
/**
 * Create-broadcast trigger. Tap → inline title prompt → creates a draft row and
...
export function NewBroadcastButton() {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await createBroadcast(value);
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Button onClick={() => { setOpen(false); setTitle(""); }} size="sm" variant="ghost" disabled={pending}>
        Cancel
      </Button>
    </div>
  );
